...
#include "../../xr_3da/fmesh.h"  // FSlideWindowItem, ogf_header, etc.
...
#include "vk_ring_allocator.h"    // CFrameRingAllocator — per-frame ring buffer
...
// ============================================================================
static constexpr u32 NUM_SUN_CASCADES = 6;   // Compile-time max (buffer sizing)
extern u32 ps_r__sun_cascades;                // Runtime active count (console var, 1-6)
...
private:
    ...
    // ========================================================================
    // Point/Spot light shadow rendering
    // ========================================================================
    void RenderLevelVisuals_Light(const Fmatrix& viewProj, const Fvector& light_pos, float light_range);
...
public:
    ...
    // ========================================================================
    // Deferred DLSS Resize (applied between frames, not mid-frame)
    // ========================================================================
    bool m_bDlssPendingResize = false;
    void ApplyDeferredDlssResize();  // Called from End() after present

    // ========================================================================
    // Render Graph + Ring Allocator (Step A)
    // ========================================================================
    VK::CFrameGraph          m_Graph;           // automatic barrier tracking
    VK::CFrameRingAllocator  m_RingAllocator;   // per-frame UBO/SSBO ring buffer

    void render_shadow_geometry(u32 cascade_ind);  // Render shadow caster geometry for cascade
    void render_shadow_geometry(const Fmatrix& viewProj, const Fvector& light_pos, float light_range);  // Point/spot shadow rendering
