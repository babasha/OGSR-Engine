...
//   Tangent  (D3DCOLOR)  @ offset 16  (W = du sub-pixel UV offset)
//   Binormal (D3DCOLOR)  @ offset 20  (W = dv sub-pixel UV offset)
    ...
    u32 tcOffset)
{
    ...
    if (stride == 32)
    {
        ...
        // Both sub-layouts have: Normal@12, Tangent@16, Binormal@20
        //   lmap (tcOffset=24): TC0@24, TC1@28
        //   vert (tcOffset=28): COLOR@24, TC0@28
        ...
        // Tangent (D3DCOLOR) @ offset 16 — .a = du (sub-pixel UV offset for DX11 unpack_tc_base)
        attributes[3].binding = 0;
        attributes[3].location = 3;
        attributes[3].format = VK_FORMAT_B8G8R8A8_UNORM;
        attributes[3].offset = 16;

        // Binormal (D3DCOLOR) @ offset 20 — .a = dv (sub-pixel UV offset for DX11 unpack_tc_base)
        attributes[4].binding = 0;
        attributes[4].location = 4;
        attributes[4].format = VK_FORMAT_B8G8R8A8_UNORM;
        attributes[4].offset = 20;

        // Vertex input state — 5 attributes for stride-32
    ...
    {
        // Layout B: non-skinned meshes with stride 36+
        // FLOAT4 pos(0) + D3DCOLOR normal(16) + D3DCOLOR tangent(20) + D3DCOLOR binormal(24) + FLOAT2 tc(28)
        ...
        // Tangent (D3DCOLOR) @ offset 20
        attributes[3].binding = 0;
        attributes[3].location = 3;
        attributes[3].format = VK_FORMAT_B8G8R8A8_UNORM;
        attributes[3].offset = 20;

        // Binormal (D3DCOLOR) @ offset 24
        attributes[4].binding = 0;
        attributes[4].location = 4;
        attributes[4].format = VK_FORMAT_B8G8R8A8_UNORM;
        attributes[4].offset = 24;

        // Vertex input state — 5 attributes for non-stride-32
        vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
        vertexInputInfo.vertexBindingDescriptionCount = 1;
        vertexInputInfo.pVertexBindingDescriptions = &binding;
        vertexInputInfo.vertexAttributeDescriptionCount = 5;
    ...
    u32 stride)
{
    ...
    else if (stride == 44)
    {
        ...
        // Attr 5: Bone index alias (D3DCOLOR) @ offset 24 — .a encodes bone index 2
        // Needed because gbuffer_skinned.vert declares layout(location = 5)
        attributes[5].binding = 0;
        attributes[5].location = 5;
        attributes[5].format = VK_FORMAT_B8G8R8A8_UNORM;
        attributes[5].offset = 24;
