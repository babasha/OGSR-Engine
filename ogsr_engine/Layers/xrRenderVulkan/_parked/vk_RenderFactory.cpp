...
public:
    ...
    void Create(HWND hwnd, u32& w, u32& h, float& fw, float& fh, bool b) override
    {
        ...
        // Ensure GPU is fully idle before first frame to prevent semaphore race
    ...
    void Begin() override
    {
        ...
        // ============================================================
        // Deferred DLSS resize — apply BEFORE acquiring swapchain image
        // or starting any command buffers.  At this point no Vulkan
        // state is active, so vkDeviceWaitIdle + Destroy/Create is safe.
        // ============================================================
        if (RImplementation.m_bDlssPendingResize) {
            RImplementation.ApplyDeferredDlssResize();
        }
        ...
        // 1.1 On the first FRAMES_IN_FLIGHT frames, fences are pre-signaled so
        // WaitForFence returns immediately. But the previous cycle's semaphore
        // may not have been consumed by the presentation engine yet. Wait idle
        // to avoid VUID-vkQueueSubmit-pSignalSemaphores-00067.
        {
            static u32 s_startupFrames = 0;
            if (s_startupFrames < CVulkanSync::FRAMES_IN_FLIGHT) {
                vkDeviceWaitIdle(VulkanHW.m_Device);
                s_startupFrames++;
            }
        }

        // 1.5 NVIDIA Reflex: sleep until GPU is ready (reduces input-to-display latency)
        if (g_ReflexManager.IsEnabled()) {
            g_ReflexManager.Sleep();
        }
    ...
    void End_Inner()
    {
        ...
        __try {
            ...
            // ================================================================
            // DLSS Frame Generation: evaluate + blit interpolated frame
            // ================================================================
            // After all rendering (scene + UI) is done, before ending cmd buffer:
            // 1. BackupRealFrame (copy swapchain → rt_RealFrame)
            // 2. Evaluate DLSS-G (generates interpolated frame)
            // 3. Blit interpolated frame → swapchain
            // 4. Transition swapchain → PRESENT_SRC
            if (ps_r__frame_gen && g_FrameGenManager.IsFeatureCreated()
                && g_FrameGenManager.HasFrameState()
                && RTarget && RTarget->m_bFrameGenBuffersReady
                && m_CurrentCmd != VK_NULL_HANDLE)
            {
                // Step 1: Backup the real frame (swapchain → rt_RealFrame)
                // After this, swapchain is in TRANSFER_DST layout
                RTarget->BackupRealFrame(m_CurrentCmd);

                // Step 2: Evaluate DLSS-G
                // Input: rt_RealFrame (backbuffer), depth, MVecs, rt_HudlessColor
                // Output: rt_InterpFrame
                u32 dispW = Swapchain.GetWidth();
                u32 dispH = Swapchain.GetHeight();
                u32 rendW = RTarget->m_Width;
                u32 rendH = RTarget->m_Height;
                VkFormat swFmt = Swapchain.GetFormat();

                g_FrameGenManager.Evaluate(m_CurrentCmd,
                    RTarget->rt_RealFrame.m_Image,     RTarget->rt_RealFrame.m_ImageView,
                    dispW, dispH, swFmt,
                    RTarget->rt_ZBuffer.m_Image,        RTarget->rt_ZBuffer.m_ImageView,
                    rendW, rendH,
                    RTarget->rt_MotionVector.m_Image,   RTarget->rt_MotionVector.m_ImageView,
                    rendW, rendH,
                    RTarget->rt_HudlessColor.m_Image,   RTarget->rt_HudlessColor.m_ImageView,
                    RTarget->rt_InterpFrame.m_Image,     RTarget->rt_InterpFrame.m_ImageView,
                    g_FrameGenManager.m_FrameJitterX,    g_FrameGenManager.m_FrameJitterY,
                    g_FrameGenManager.m_FrameProj,       g_FrameGenManager.m_FrameView,
                    g_FrameGenManager.m_FrameProj,       g_FrameGenManager.m_FrameView,  // prev = curr (internal tracking handles it)
                    g_FrameGenManager.m_FrameCamPos,
                    g_FrameGenManager.m_FrameNear,       g_FrameGenManager.m_FrameFar,
                    g_FrameGenManager.m_FrameFOV,        g_FrameGenManager.m_FrameAspect);

                // Step 3: Blit rt_InterpFrame → swapchain
                // rt_InterpFrame is in whatever layout NGX left it; transition to TRANSFER_SRC
                VkImage swapImg = Swapchain.GetCurrentImage();
                {
                    VkImageMemoryBarrier bar = {};
                    bar.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
                    ...
                    bar.oldLayout = VK_IMAGE_LAYOUT_GENERAL;  // NGX typically leaves images in GENERAL
                    bar.newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
                    bar.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
                    bar.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
                    bar.image = RTarget->rt_InterpFrame.m_Image;
                ...
                // Copy rt_InterpFrame → swapchain (already in TRANSFER_DST from BackupRealFrame)
                VkImageCopy region = {};
                region.srcSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 };
                region.dstSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 };
                region.extent = { dispW, dispH, 1 };
                vkCmdCopyImage(m_CurrentCmd,
                    RTarget->rt_InterpFrame.m_Image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                    swapImg,                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                    1, &region);

                // Step 4: Transition swapchain → PRESENT_SRC
                {
                    VkImageMemoryBarrier bar = {};
                    bar.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
                    bar.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
                ...
                g_FrameGenManager.ClearFrameState();
            }
            ...
            end_step = 7;
            // NVIDIA Reflex: mark render submit end
            g_ReflexManager.SetMarker(VK_RENDERSUBMIT_END);
            ...
            end_step = 8;
            // NVIDIA Reflex: mark present start
            g_ReflexManager.SetMarker(VK_PRESENT_START);

            // Present the rendered image (interpolated frame when frame gen is active)
            Swapchain.Present(sync.renderFinished, m_CurrentImageIndex);

            // Double-present: show real frame after interpolated frame
            // This doubles the effective present rate (like Cyberpunk 2077's DLSS-G)
            if (ps_r__frame_gen && g_FrameGenManager.IsFeatureCreated()
                && RTarget && RTarget->m_bFrameGenBuffersReady
                && RTarget->rt_RealFrame.m_Image != VK_NULL_HANDLE)
            {
                g_FrameGenManager.PresentRealFrame(
                    RTarget->rt_RealFrame.m_Image,
                    Swapchain.GetWidth(), Swapchain.GetHeight());
            }

            // NVIDIA Reflex: mark present end + simulation end
            g_ReflexManager.SetMarker(VK_PRESENT_END);
            g_ReflexManager.SetMarker(VK_SIMULATION_END);
        ...
        // NOTE: Deferred DLSS resize moved to Begin() — see Begin() override
