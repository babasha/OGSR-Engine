...
void CBackend::SetBoneRegion(VK::RingAlloc alloc)
{
    if (alloc.IsValid())
    {
        m_BoneMapped         = (Fmatrix*)alloc.ptr;
        m_BoneBuffer_Current = alloc.buffer;
        m_BoneOffset_Current = alloc.offset;
    }
    else
    {
        // Fallback: legacy persistent buffer
        m_BoneMapped         = m_BoneBuffer.IsValid() ? (Fmatrix*)m_BoneBuffer.m_Mapped : nullptr;
        m_BoneBuffer_Current = m_BoneBuffer.IsValid() ? m_BoneBuffer.m_Buffer : VK_NULL_HANDLE;
        m_BoneOffset_Current = 0;
    }
}
...
void CBackend::set_ca(void* c, u32 startReg, u32 count, const void* data)
{
    ...
    // Ring buffer is HOST_COHERENT — no explicit flush needed
}
