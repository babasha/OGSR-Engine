...
void CVulkanGeometry::CreateFullscreenQuad()
{
    ...
    // Vulkan clip space: Y=-1 is TOP of screen, Y=+1 is BOTTOM
    // Texture UV: V=0 is TOP of texture, V=1 is BOTTOM
    // So clip Y=-1 (top) → UV V=0 (top), clip Y=+1 (bottom) → UV V=1 (bottom)
    Vertex vertices[4] = {
        {-1.0f, -1.0f, 0.0f, 1.0f,  0.0f, 0.0f},  // Top-left     (clip Y=-1 = screen top)
        {-1.0f,  1.0f, 0.0f, 1.0f,  0.0f, 1.0f},  // Bottom-left  (clip Y=+1 = screen bottom)
