...
void R_dsgraph_structure::r_dsgraph_insert_static(dxRender_Visual* pVisual)
{
    ...
    // Skip trees when GPU-driven indirect rendering is active
    vkRender_Visual* vkV = reinterpret_cast<vkRender_Visual*>(pVisual);
    if ((vkV->Type == MT_TREE_ST || vkV->Type == MT_TREE_PM) &&
        VK::g_ShadowManager && VK::g_ShadowManager->IsTreeGBufferReady())
        return;
...
void CLight_DB::Load(IReader* fs)
{
    ...
    if (!F) {
        Msg("! [Light_DB] fsL_LIGHT_DYNAMIC chunk not found");
        return;
    }
    ...
    for (u32 i = 0; i < count; i++) {
        ...
        if (Ldata.type == D3DLIGHT_DIRECTIONAL) {
            ...
            Msg("[Vulkan] Sun light loaded (directional)");
        } else {
            Fvector tmp_D, tmp_R;
            tmp_D.set(0, 0, -1);
            tmp_R.set(1, 0, 0);
    ...
    if (!sun_original || !sun_adapted)
        Msg("! [Light_DB] No directional light (sun) found in level");
    else
        Msg("[Vulkan] Light_DB loaded: %u static lights + sun", (u32)v_static.size());
}
...
void CLight_DB::Update()
{
    ...
    if (sun_original && sun_adapted)
    {
        ...
        // HARDCODED FALLBACK (TODO: убрать когда будет поддержка лайтмап)
        // Anomaly weather configs часто ставят sun_color=0 для облачной/пасмурной погоды.
        // DX11 компенсирует это лайтмапами (запечённое освещение в уровне).
        // Vulkan рендерер не поддерживает лайтмапы, поэтому без этого хака сцена полностью тёмная.
        // Значения яркости (0.3-0.8) и тёплый оттенок (0.95, 0.85) подобраны на глаз.
        Fvector sun_col = E.sun_color;
        float elevation = -E.sun_dir.y;  // Positive when sun above horizon
        if (elevation > 0.05f) {
            float mag = sun_col.x + sun_col.y + sun_col.z;
            if (mag < 0.01f) {
                float t = (elevation < 0.7f) ? (elevation / 0.7f) : 1.0f;
                float bright = 0.3f + 0.5f * t;  // 0.3 at horizon → 0.8 at zenith
                sun_col.set(bright, bright * 0.95f, bright * 0.85f);  // Warm tint
            }
        }

        sun_original->set_rotation(OD, _sun_original->right);
        sun_original->set_position(OP);
        sun_original->set_color(sun_col.x, sun_col.y, sun_col.z);
        sun_original->set_range(600.f);

        sun_adapted->set_rotation(AD, _sun_adapted->right);
        sun_adapted->set_position(AP);
        sun_adapted->set_color(sun_col.x * ps_r2_sun_lumscale, sun_col.y * ps_r2_sun_lumscale,
                               sun_col.z * ps_r2_sun_lumscale);
        sun_adapted->set_range(600.f);

        // DIAGNOSTIC: trace sun_color source + weather lerp inputs
        static u32 s_sunDiagFrame = 0;
        if (Device.dwFrame - s_sunDiagFrame > 300) {
            s_sunDiagFrame = Device.dwFrame;
            light* sa = (light*)sun_adapted._get();
            CEnvironment& env = g_pGamePersistent->Environment();
            bool fallback = (E.sun_color.x + E.sun_color.y + E.sun_color.z < 0.01f) && (-E.sun_dir.y > 0.05f);
            Msg("[SUN-DIAG] E.sun_color=(%.4f,%.4f,%.4f) actual=(%.4f,%.4f,%.4f)%s dir=(%.3f,%.3f,%.3f) gameTime=%.0f",
                E.sun_color.x, E.sun_color.y, E.sun_color.z,
                sun_col.x, sun_col.y, sun_col.z,
                fallback ? " FALLBACK" : "",
                E.sun_dir.x, E.sun_dir.y, E.sun_dir.z,
                env.GetGameTime());
            if (env.Current[0] && env.Current[1]) {
                Msg("[SUN-DIAG] A='%s' sun=(%.3f,%.3f,%.3f) B='%s' sun=(%.3f,%.3f,%.3f)",
                    env.Current[0]->m_identifier.c_str(),
                    env.Current[0]->sun_color.x, env.Current[0]->sun_color.y, env.Current[0]->sun_color.z,
                    env.Current[1]->m_identifier.c_str(),
                    env.Current[1]->sun_color.x, env.Current[1]->sun_color.y, env.Current[1]->sun_color.z);
            }
        }
