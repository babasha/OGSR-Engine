...
#include "vk_command_buffer.h"  // For CommandManager.GetCurrentFrame()
#include "../../xr_3da/igame_persistent.h"  // For g_pGamePersistent and Environment access
#include "../../xr_3da/Environment.h"       // For CEnvDescriptor
...
void CVulkanLighting::Create()
{
    ...
    VkDescriptorPoolSize poolSizes[] = {
        { VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 20 },  // G-Buffer textures + per-frame shadow samplers
        { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 14 }            // 3x GlobalLighting + 3x Material + per-frame SunData UBOs
    };

    VkDescriptorPoolCreateInfo poolInfo = {};
    poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    poolInfo.maxSets = 18;  // G-Buffer + 3x GlobalLighting + 3x Material + 3x shadow sets + headroom
    ...
    // 3c. Create sun shadow resources (Phase 1)
    CreateSunShadowResources();

    // 3d. Build constant name → offset mapping
    BuildConstantMap();

    // 4. Create G-Buffer descriptor set layout
    CreateGBufferDescriptorSetLayout();

    // 5. Shader loading deferred to LoadShadersDeferred() (g_ShaderManager not yet available)

    m_bCreated = true;
    Msg("[Vulkan] CVulkanLighting created successfully (shaders deferred)");
}

void CVulkanLighting::LoadShadersDeferred()
{
    if (!m_bCreated) {
        Msg("![Vulkan] LoadShadersDeferred: CVulkanLighting not created yet");
        return;
    }
    if (m_bShadersLoaded) {
    ...
    Msg("[Vulkan] LoadShadersDeferred: loading shaders...");
...
void CVulkanLighting::Destroy()
{
    ...
    // Destroy sun shadow resources (Phase 1)
...
bool CVulkanLighting::CreateAccumDirectSimplePipeline()
{
    ...
    pipelineRenderingInfo.depthAttachmentFormat = Swapchain.m_DepthFormat;
    pipelineRenderingInfo.stencilAttachmentFormat =
        (Swapchain.m_DepthFormat == VK_FORMAT_D32_SFLOAT_S8_UINT ||
         Swapchain.m_DepthFormat == VK_FORMAT_D24_UNORM_S8_UINT)
        ? Swapchain.m_DepthFormat : VK_FORMAT_UNDEFINED;
                                             ...
                                             float L_spec)
{
    ...
    // ========================================================================
    // 2. Bind descriptor sets (only sets defined in pipeline layout)
    // ========================================================================

    u32 fi = CommandManager.GetCurrentFrame() % LIGHTING_FRAMES;
    VkDescriptorSet sets[2] = {
        m_GlobalLightingDescriptorSet[fi],  // Set 0: GlobalLighting UBO
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            ...
                            0,  // firstSet
                            2,  // descriptorSetCount (must match pipeline layout setLayoutCount)
                            sets,
...
void CVulkanLighting::CreateGlobalLightingUBO()
{
    ...
    // ========================================================================
    // 5. Allocate Descriptor Sets (one per frame-in-flight)
    // ========================================================================
    VkDescriptorSetLayout layouts[LIGHTING_FRAMES];
    for (u32 i = 0; i < LIGHTING_FRAMES; i++)
        layouts[i] = m_GlobalLightingLayout;
    ...
    allocDescInfo.descriptorSetCount = LIGHTING_FRAMES;
    allocDescInfo.pSetLayouts = layouts;

    result = vkAllocateDescriptorSets(VulkanHW.m_Device, &allocDescInfo, m_GlobalLightingDescriptorSet);
    if (result != VK_SUCCESS) {
        Msg("![Vulkan] Failed to allocate GlobalLighting descriptor sets: %d", result);
    ...
    // ========================================================================
    // 6. Initial update of all descriptor sets with legacy buffer
    // ========================================================================
    ...
    for (u32 i = 0; i < LIGHTING_FRAMES; i++) {
        ...
        descriptorWrite.dstSet = m_GlobalLightingDescriptorSet[i];
    ...
    Msg("[Vulkan] GlobalLighting UBO created successfully (%u per-frame desc sets)", LIGHTING_FRAMES);
...
void CVulkanLighting::UpdateGlobalLightingUBO(RingAlloc alloc)
{
    if (!alloc.IsValid() && !m_GlobalLightingMapped)
        return;
    ...
    // ========================================================================
    // Get current view matrix from Device and compute inverse
    m_invV.invert(Device.mView);  // m_invV = inverse(Device.mView)
    ubo.m_invV = m_invV;
    ...
    // actual object motion, not the jitter delta.
    {
        ...
        // previous.x/y == 0 when jitter was disabled last frame, so always safe to add
    ...
    // ========================================================================
    // Copy to mapped memory and update descriptor set
    // ========================================================================
    u32 frameIndex = CommandManager.GetCurrentFrame() % LIGHTING_FRAMES;

    if (alloc.IsValid())
    {
        memcpy(alloc.ptr, &ubo, sizeof(ubo));

        VkDescriptorBufferInfo bufInfo = {};
        bufInfo.buffer = alloc.buffer;
        bufInfo.offset = alloc.offset;
        bufInfo.range  = sizeof(GlobalLightingUBO);

        VkWriteDescriptorSet w = {};
        w.sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        w.dstSet          = m_GlobalLightingDescriptorSet[frameIndex];
        w.dstBinding      = 0;
    ...
    {
        // Fallback: legacy persistent buffer (remove after ring-allocator validation)
        memcpy(m_GlobalLightingMapped, &ubo, sizeof(ubo));
    }
}
...
void CVulkanLighting::DestroyGlobalLightingUBO()
{
    ...
    // Descriptor sets automatically freed when pool is destroyed
    for (u32 i = 0; i < LIGHTING_FRAMES; i++)
        m_GlobalLightingDescriptorSet[i] = VK_NULL_HANDLE;
...
void CVulkanLighting::CreateMaterialConstantsUBO()
{
    ...
    // 5. Allocate Descriptor Sets (one per frame-in-flight)
    // ========================================================================
    VkDescriptorSetLayout matLayouts[LIGHTING_FRAMES];
    for (u32 i = 0; i < LIGHTING_FRAMES; i++)
        matLayouts[i] = m_MaterialConstantsLayout;

    VkDescriptorSetAllocateInfo allocSetInfo = {};
    allocSetInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocSetInfo.descriptorPool = m_DescriptorPool;
    allocSetInfo.descriptorSetCount = LIGHTING_FRAMES;
    allocSetInfo.pSetLayouts = matLayouts;

    result = vkAllocateDescriptorSets(VulkanHW.m_Device, &allocSetInfo, m_MaterialConstantsDescriptorSet);
    if (result != VK_SUCCESS) {
        Msg("![Vulkan] Failed to allocate MaterialConstants descriptor sets: %d", result);
    ...
    for (u32 i = 0; i < LIGHTING_FRAMES; i++) {
        VkWriteDescriptorSet descriptorWrite = {};
        descriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrite.dstSet = m_MaterialConstantsDescriptorSet[i];
        descriptorWrite.dstBinding = 0;
        descriptorWrite.dstArrayElement = 0;
        descriptorWrite.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        descriptorWrite.descriptorCount = 1;
    ...
    Msg("[Vulkan] MaterialConstants UBO created successfully (%u per-frame desc sets)", LIGHTING_FRAMES);
...
void CVulkanLighting::UpdateMaterialConstantsUBO(RingAlloc alloc)
{
    if (!alloc.IsValid() && !m_MaterialConstantsMapped)
        return;
    ...
    // ========================================================================
    // Copy to mapped memory and update descriptor set (current frame only)
    // ========================================================================
    u32 frameIndex = CommandManager.GetCurrentFrame() % LIGHTING_FRAMES;
    ...
    if (alloc.IsValid())
    {
        ...
        bufInfo.buffer = alloc.buffer;
        bufInfo.offset = alloc.offset;
        bufInfo.range  = sizeof(MaterialConstantsUBO);

        VkWriteDescriptorSet w = {};
        w.sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        w.dstSet          = m_MaterialConstantsDescriptorSet[frameIndex];
        w.dstBinding      = 0;
        w.descriptorType  = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        w.descriptorCount = 1;
        w.pBufferInfo     = &bufInfo;
    ...
    {
        memcpy(m_MaterialConstantsMapped, &ubo, sizeof(ubo));
    }
}
...
void CVulkanLighting::DestroyMaterialConstantsUBO()
{
    ...
    // Descriptor sets automatically freed when pool is destroyed
    for (u32 i = 0; i < LIGHTING_FRAMES; i++)
        m_MaterialConstantsDescriptorSet[i] = VK_NULL_HANDLE;
...
// ============================================================================
// Sun Cascade Shadow Maps Implementation
// ============================================================================

void CVulkanLighting::CreateSunShadowResources()
{
	Msg("[Vulkan] Creating sun shadow resources (Set 3)...");

	// ========================================================================
	// Step 1: Create SunData UBO (544 bytes, 6 cascades)
	// ========================================================================
	VkBufferCreateInfo bufferInfo = {};
	bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
	bufferInfo.size = sizeof(SunDataUBO);
	bufferInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
	bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

	VkResult result = vkCreateBuffer(VulkanHW.m_Device, &bufferInfo, nullptr, &m_SunDataBuffer);
	if (result != VK_SUCCESS) {
		Msg("![Vulkan] Failed to create SunData UBO buffer: %d", result);
		return;
	}

	// Allocate memory (HOST_VISIBLE + HOST_COHERENT for persistent mapping)
	VkMemoryRequirements memReq;
	vkGetBufferMemoryRequirements(VulkanHW.m_Device, m_SunDataBuffer, &memReq);
	...
	if (memoryTypeIndex == UINT32_MAX) {
		Msg("![Vulkan] Failed to find suitable memory type for SunData UBO");
		vkDestroyBuffer(VulkanHW.m_Device, m_SunDataBuffer, nullptr);
		m_SunDataBuffer = VK_NULL_HANDLE;
		return;
	}
	...
	result = vkAllocateMemory(VulkanHW.m_Device, &allocInfo, nullptr, &m_SunDataMemory);
	if (result != VK_SUCCESS) {
		Msg("![Vulkan] Failed to allocate SunData UBO memory: %d", result);
		vkDestroyBuffer(VulkanHW.m_Device, m_SunDataBuffer, nullptr);
		m_SunDataBuffer = VK_NULL_HANDLE;
		return;
	}

	vkBindBufferMemory(VulkanHW.m_Device, m_SunDataBuffer, m_SunDataMemory, 0);

	// Persistent mapping
	vkMapMemory(VulkanHW.m_Device, m_SunDataMemory, 0, bufferInfo.size, 0, &m_SunDataMapped);
	// Initialize active pointers to legacy buffer (ring path will override each frame)
	m_SunDataActiveBuffer = m_SunDataBuffer;
	m_SunDataActiveOffset = 0;
	Msg("[Vulkan] SunData UBO created and mapped");

	// ========================================================================
	// Step 2: Create shadow comparison sampler
	// ========================================================================
	VkSamplerCreateInfo samplerInfo = {};
	samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
	...
	samplerInfo.mipLodBias = 0.0f;
	samplerInfo.maxAnisotropy = 1.0f;
	samplerInfo.compareEnable = VK_FALSE;  // Manual comparison in shader (AMD amdvlk crashes with hardware comparison)
	samplerInfo.compareOp = VK_COMPARE_OP_NEVER;
	samplerInfo.minLod = 0.0f;
	samplerInfo.maxLod = 1.0f;
	samplerInfo.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;  // Out of bounds = fully lit

	result = vkCreateSampler(VulkanHW.m_Device, &samplerInfo, nullptr, &m_ShadowSampler);
	if (result != VK_SUCCESS) {
		Msg("![Vulkan] Failed to create shadow comparison sampler: %d", result);
		return;
	}
	Msg("[Vulkan] Shadow sampler created (PCF compare)");

	// ========================================================================
	// Step 3: Create descriptor set layout (Set 3)
	// ========================================================================
	VkDescriptorSetLayoutBinding bindings[2] = {};

	// Binding 0: Shadow map sampler2DShadow
	bindings[0].binding = 0;
	bindings[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	bindings[0].descriptorCount = 1;
	bindings[0].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
	bindings[0].pImmutableSamplers = nullptr;  // Sampler provided at descriptor update time

	// Binding 1: SunData UBO
	bindings[1].binding = 1;
	...
	result = vkCreateDescriptorSetLayout(VulkanHW.m_Device, &layoutInfo, nullptr, &m_SunShadowLayout);
	if (result != VK_SUCCESS) {
		Msg("![Vulkan] Failed to create sun shadow descriptor set layout: %d", result);
		return;
	}
	Msg("[Vulkan] Sun shadow descriptor set layout created");

	// ========================================================================
	// Step 4: Allocate per-frame descriptor sets from pool
	// ========================================================================
	VkDescriptorSetLayout layouts[SUN_SHADOW_FRAMES];
	for (u32 i = 0; i < SUN_SHADOW_FRAMES; i++)
		layouts[i] = m_SunShadowLayout;

	VkDescriptorSetAllocateInfo allocDescInfo = {};
	allocDescInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocDescInfo.descriptorPool = m_DescriptorPool;
	allocDescInfo.descriptorSetCount = SUN_SHADOW_FRAMES;
	allocDescInfo.pSetLayouts = layouts;

	result = vkAllocateDescriptorSets(VulkanHW.m_Device, &allocDescInfo, m_SunShadowDescSet);
	if (result != VK_SUCCESS) {
		Msg("![Vulkan] Failed to allocate sun shadow descriptor sets: %d", result);
		return;
	}
	Msg("[Vulkan] Sun shadow descriptor sets allocated (%u)", SUN_SHADOW_FRAMES);

	// Note: Pipeline creation deferred to CreateCascadePipelineLazy()
	// because g_ShaderManager may not be initialized yet at this point.

	Msg("[Vulkan] Sun shadow resources created (pipeline deferred)");
}

void CVulkanLighting::DestroySunShadowResources()
{
	Msg("[Vulkan] Destroying sun shadow resources...");

	if (m_AccumCascade_Pipeline != VK_NULL_HANDLE) {
		vkDestroyPipeline(VulkanHW.m_Device, m_AccumCascade_Pipeline, nullptr);
		m_AccumCascade_Pipeline = VK_NULL_HANDLE;
	}

	if (m_AccumCascade_PipelineLayout != VK_NULL_HANDLE) {
		vkDestroyPipelineLayout(VulkanHW.m_Device, m_AccumCascade_PipelineLayout, nullptr);
		m_AccumCascade_PipelineLayout = VK_NULL_HANDLE;
	}

	if (m_EmptyLayout != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(VulkanHW.m_Device, m_EmptyLayout, nullptr);
		m_EmptyLayout = VK_NULL_HANDLE;
	}

	// Note: shader modules are owned by g_ShaderManager (cached), don't destroy here
	m_AccumCascade_VS = VK_NULL_HANDLE;
	m_AccumCascade_FS = VK_NULL_HANDLE;

	// Note: m_SunShadowDescSet will be destroyed automatically when descriptor pool is destroyed

	if (m_SunShadowLayout != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(VulkanHW.m_Device, m_SunShadowLayout, nullptr);
		m_SunShadowLayout = VK_NULL_HANDLE;
	}

	if (m_ShadowSampler != VK_NULL_HANDLE) {
		vkDestroySampler(VulkanHW.m_Device, m_ShadowSampler, nullptr);
		m_ShadowSampler = VK_NULL_HANDLE;
	}

	if (m_SunDataMapped != nullptr) {
		vkUnmapMemory(VulkanHW.m_Device, m_SunDataMemory);
		m_SunDataMapped = nullptr;
	}

	if (m_SunDataMemory != VK_NULL_HANDLE) {
		vkFreeMemory(VulkanHW.m_Device, m_SunDataMemory, nullptr);
		m_SunDataMemory = VK_NULL_HANDLE;
	}

	if (m_SunDataBuffer != VK_NULL_HANDLE) {
		vkDestroyBuffer(VulkanHW.m_Device, m_SunDataBuffer, nullptr);
		m_SunDataBuffer = VK_NULL_HANDLE;
	}

	Msg("[Vulkan] Sun shadow resources destroyed");
}

void CVulkanLighting::UpdateSunDataUBO(const Fvector& L_dir, const Fvector& L_color, float L_spec)
{
	static u32 s_callCount = 0;
	if (s_callCount < 3) {
		Msg("[UpdateSunDataUBO] Called (frame %u), m_SunDataMapped=%p", Device.dwFrame, m_SunDataMapped);
		s_callCount++;
	}

	// DIAGNOSTIC: Log Device.mInvView and Device.mView matrices
	static u32 s_matrixLogged = 0;
	if (s_matrixLogged < 3) {
		Msg("[MATRIX-DIAG] Device.mView[0][0] = %.3f, mInvView[0][0] = %.3f",
		    Device.mView.m[0][0], Device.mInvView.m[0][0]);
		s_matrixLogged++;
	}

	SunDataUBO sunData_local = {};
	SunDataUBO* sunData = &sunData_local;

	// Sun direction (view space) - already computed by caller
	sunData->Ldynamic_dir.set(L_dir.x, L_dir.y, L_dir.z, 0.0f);

	// Sun color + specular
	sunData->Ldynamic_color.set(L_color.x, L_color.y, L_color.z, L_spec);

	// Cascade matrices: world → light clip (NOT eye→light-clip!)
	// The shader will do eye→world using globals.m_invV (from GlobalLighting UBO),
	// then world→light-clip using m_shadow[i]. This guarantees consistency with
	// the same inverse view matrix used by the gbuffer.
	u32 activeCascades = _min(ps_r__sun_cascades, (u32)RImplementation.m_sun_cascades.size());
	for (u32 i = 0; i < NUM_SUN_CASCADES; i++) {
		if (i < activeCascades) {
			sunData->m_shadow[i] = RImplementation.m_sun_cascades[i].xform;
		} else {
			sunData->m_shadow[i].identity();
		}
	}

	// Cascade split distances (packed into 2 vec4s)
	// splits[0].xyzw = cascade 0-3 far distances
	// splits[1].xy   = cascade 4-5 far distances, zw = 0
	// Inactive cascades get 99999.0f so the shader never selects them
	auto cascadeDist = [&](u32 idx, float fallback) -> float {
		if (idx < activeCascades && idx < (u32)RImplementation.m_sun_cascades.size())
			return RImplementation.m_sun_cascades[idx].size;
		return (idx < activeCascades) ? fallback : 99999.0f;
	};
	sunData->cascade_splits[0].set(
		cascadeDist(0, 10.0f),
		cascadeDist(1, 25.0f),
		cascadeDist(2, 60.0f),
		cascadeDist(3, 150.0f)
	);
	sunData->cascade_splits[1].set(
		cascadeDist(4, 275.0f),
		cascadeDist(5, 400.0f),
		0.0f, 0.0f
	);

	// Atlas parameters (sub-region coordinates within 3072x2048 shadow atlas)
	// 3x2 grid of 1024x1024 tiles
	// atlas_params[i] = (offsetU, offsetV, scaleU, scaleV)
	const float atlasW = 3072.0f;
	const float atlasH = 2048.0f;
	const float tileSize = 1024.0f;
	for (u32 i = 0; i < NUM_SUN_CASCADES; i++) {
		u32 col = i % 3;
		u32 row = i / 3;
		sunData->atlas_params[i].set(
			(float)(col) * tileSize / atlasW,   // offsetU
			(float)(row) * tileSize / atlasH,   // offsetV
			tileSize / atlasW,                   // scaleU
			tileSize / atlasH                    // scaleV
		);
	}

	// Write to ring if available, else fallback to legacy mapped buffer
	RingAlloc alloc = RImplementation.m_RingAllocator.Allocate(sizeof(SunDataUBO), 256);
	if (alloc.IsValid())
	{
		memcpy(alloc.ptr, &sunData_local, sizeof(sunData_local));
		m_SunDataActiveBuffer = alloc.buffer;
		m_SunDataActiveOffset = alloc.offset;
	}
	else if (m_SunDataMapped)
	{
		memcpy(m_SunDataMapped, &sunData_local, sizeof(sunData_local));
		m_SunDataActiveBuffer = m_SunDataBuffer;
		m_SunDataActiveOffset = 0;
	}
}

bool CVulkanLighting::CreateCascadePipelineLazy()
{
	if (m_AccumCascade_Pipeline != VK_NULL_HANDLE)
		return true;  // Already created

	if (!g_ShaderManager) {
		Msg("![Vulkan] CreateCascadePipelineLazy: g_ShaderManager is NULL");
		return false;
	}
	if (!m_SunShadowLayout) {
		Msg("![Vulkan] CreateCascadePipelineLazy: m_SunShadowLayout is NULL");
		return false;
	}
	if (!m_GlobalLightingLayout) {
		Msg("![Vulkan] CreateCascadePipelineLazy: m_GlobalLightingLayout is NULL");
		return false;
	}
	if (!m_GBufferLayout) {
		Msg("![Vulkan] CreateCascadePipelineLazy: m_GBufferLayout is NULL");
		return false;
	}

	Msg("[Vulkan] Creating cascade pipeline (deferred)...");

	// Load cascade shaders
	m_AccumCascade_VS = g_ShaderManager->Load("vulkan/accum_sun_cascades.vert.spv");
	m_AccumCascade_FS = g_ShaderManager->Load("vulkan/accum_sun_cascades.frag.spv");
	if (m_AccumCascade_VS == VK_NULL_HANDLE || m_AccumCascade_FS == VK_NULL_HANDLE) {
		Msg("![Vulkan] Failed to load cascade shaders");
		return false;
	}

	// Create empty descriptor set layout (placeholder for Set 2)
	{
		VkDescriptorSetLayoutCreateInfo emptyInfo = {};
		emptyInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
		emptyInfo.bindingCount = 0;
		VkResult r = vkCreateDescriptorSetLayout(VulkanHW.m_Device, &emptyInfo, nullptr, &m_EmptyLayout);
		if (r != VK_SUCCESS) {
			Msg("![Vulkan] Failed to create empty layout: %d", r);
			return false;
		}
	}

	// Pipeline layout: Set 0 (GlobalLighting) + Set 1 (GBuffer) + Set 2 (empty) + Set 3 (SunShadow)
	{
		...
		VkResult r = vkCreatePipelineLayout(VulkanHW.m_Device, &layoutInfo, nullptr, &m_AccumCascade_PipelineLayout);
		if (r != VK_SUCCESS) {
			Msg("![Vulkan] Failed to create cascade pipeline layout: %d", r);
			return false;
		}
	}

	// Graphics pipeline (same as accum_direct_simple but cascade shaders + 4-set layout)
	{
		VkPipelineShaderStageCreateInfo stages[2] = {};
		stages[0].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		stages[0].stage = VK_SHADER_STAGE_VERTEX_BIT;
		stages[0].module = m_AccumCascade_VS;
		stages[0].pName = "main";
		stages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		stages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
		stages[1].module = m_AccumCascade_FS;
		stages[1].pName = "main";
		...
		ds.stencilTestEnable = VK_FALSE;  // D32_SFLOAT has no stencil; shader checks P.w==0 for sky
		...
		ri.depthAttachmentFormat = Swapchain.m_DepthFormat;
		...
		pi.layout = m_AccumCascade_PipelineLayout;

		VkResult r = vkCreateGraphicsPipelines(VulkanHW.m_Device, VK_NULL_HANDLE, 1, &pi, nullptr, &m_AccumCascade_Pipeline);
		if (r != VK_SUCCESS) {
			Msg("![Vulkan] Failed to create cascade pipeline: %d", r);
			return false;
		}
	}

	Msg("[Vulkan] Cascade shadow pipeline created successfully");
	return true;
}

bool CVulkanLighting::BindAccumDirectCascades(VkCommandBuffer cmd, CRenderTarget* rt)
{
	// Lazy-init pipeline (g_ShaderManager not available during CreateSunShadowResources)
	if (m_AccumCascade_Pipeline == VK_NULL_HANDLE) {
		static bool s_tried = false;
		if (!s_tried) {
			s_tried = true;
			if (!CreateCascadePipelineLazy()) {
				Msg("![Vulkan] BindAccumDirectCascades: lazy pipeline creation failed");
				return false;
			}
		} else {
	...
	// Ensure GBuffer descriptor set exists
	if (!m_bDescriptorSetUpdated) {
		CreateGBufferDescriptorSet(rt);
		if (!m_bDescriptorSetUpdated) {
			return false;
	...
	// Bind cascade pipeline
	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, m_AccumCascade_Pipeline);

	// Update shadow descriptor set (Set 3)
	if (!rt->rt_smap_depth.m_Image || rt->rt_smap_depth.m_ImageView == VK_NULL_HANDLE) {
		return false;
	}

	// Use per-frame descriptor set to avoid updating a set still in use by GPU
	u32 frameIndex = CommandManager.GetCurrentFrame();
	VkDescriptorSet currentShadowDescSet = m_SunShadowDescSet[frameIndex % SUN_SHADOW_FRAMES];

	VkDescriptorImageInfo shadowImageInfo = {};
	shadowImageInfo.imageView = rt->rt_smap_depth.m_ImageView;
	shadowImageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	shadowImageInfo.sampler = m_ShadowSampler;

	VkDescriptorBufferInfo sunDataInfo = {};
	sunDataInfo.buffer = m_SunDataActiveBuffer != VK_NULL_HANDLE ? m_SunDataActiveBuffer : m_SunDataBuffer;
	sunDataInfo.offset = m_SunDataActiveBuffer != VK_NULL_HANDLE ? m_SunDataActiveOffset : 0;
	sunDataInfo.range = sizeof(SunDataUBO);

	VkWriteDescriptorSet writes[2] = {};
	writes[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
	writes[0].dstSet = currentShadowDescSet;
	...
	writes[1].dstSet = currentShadowDescSet;
	writes[1].dstBinding = 1;
	writes[1].descriptorCount = 1;
	writes[1].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
	writes[1].pBufferInfo = &sunDataInfo;

	vkUpdateDescriptorSets(VulkanHW.m_Device, 2, writes, 0, nullptr);

	// Bind descriptor sets: Set 0 + Set 1 together, then Set 3
	u32 fi = frameIndex % LIGHTING_FRAMES;
	VkDescriptorSet sets01[2] = {
		m_GlobalLightingDescriptorSet[fi],  // Set 0: GlobalLighting UBO (per-frame)
		m_GBufferDescriptorSet              // Set 1: G-Buffer textures
	};
	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
							m_AccumCascade_PipelineLayout,
							0, 2, sets01,
							0, nullptr);

	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
							m_AccumCascade_PipelineLayout,
							3, 1, &currentShadowDescSet,
							0, nullptr);

	return true;
}

VkDescriptorSet CVulkanLighting::GetGlobalLightingDescriptorSet() const
{
	u32 fi = CommandManager.GetCurrentFrame() % LIGHTING_FRAMES;
	return m_GlobalLightingDescriptorSet[fi];
}

VkDescriptorSet CVulkanLighting::GetMaterialConstantsDescriptorSet() const
{
	u32 fi = CommandManager.GetCurrentFrame() % LIGHTING_FRAMES;
	return m_MaterialConstantsDescriptorSet[fi];
}
