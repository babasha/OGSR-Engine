#version 450
// xrRenderVulkan — sun shadow caster for SKINNED dynamics (depth-only).
// Same GPU skinning as skinned.vert (same SkinPush block, same bone SSBO);
// bones are uploaded PRE-MULTIPLIED by the object's world matrix, so S*pos is
// world-space and pc.mvp here is the plain light view·proj. No fragment stage.
...
layout(push_constant) uniform PC {
    mat4  mvp;       // light view·proj (bones are already world-space)
