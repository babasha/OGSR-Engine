...
#include "vsm_sample.glsl"   // vsmSunShadow (Phase 1C receiver: set 1 bindings 14-16)
...
// between fragment and light — where the surface rises above the segment, the
// light is blocked. Stops a basement lamp lighting the earth + fence overhead.
// Above-ground lights early-out after one tap (not buried → full light).
float lightTerrainOcc(vec3 wp, vec3 lpos)
{
    ...
    // The light is buried. A fragment ALSO below the surface (same underground
    // pocket — a heightfield can't see the cavity) stays LIT (basement works); a
    // fragment AT/ABOVE the surface is only reachable THROUGH the ground → occlude.
...
// GTAO bent normal: the average UNOCCLUDED direction (gba channels of the AO RT,
// world-space). Sampling the sky cube along it instead of the geometric normal
// makes the ambient fill come from where the hemisphere is actually open
// (recesses pull their fill from the opening). Falls back to the geometric
// normal when AO is off/not-ready (ao_params.z==0) or the bent normal is
// degenerate (sky / unwritten texels encode ~0).
...
// Shade ONE dynamic point/spot light: linear-squared falloff, N·L diffuse,
// smooth spot cone, + the two shadow-budget lights' shadow maps & cookie. The
// light fields are passed in so this serves BOTH the UBO array (fallback) and
// the clustered SSBO. `gi` = the light's GLOBAL index (for the shadow picks).
...
// Dynamic light accumulation (STEP 3). Clustered (r_clustered): iterate only the
// froxel's lights from the SSBO — the cap is 256, each pixel touches the ~few
// lights reaching it. Fallback: the old per-fragment loop over the UBO's 16.
...
// Wet-surface contribution — SAME model as world_terrain. Open rain-exposed ground
// reads NORMAL (darken/reflection are PUDDLE-gated, not global — that global wetness
// was the "whole ground is a water layer" look). Reflection lives ONLY in puddles.
vec3 applyWetness(inout vec3 albedo, vec3 wp, vec3 N, float sunMask)
{
    ...
    // Kill wetness on DOWN-facing surfaces (ceilings, undersides) — rain can't land.
    float wetK = wet * mix(0.35, 1.0, upness) * smoothstep(-0.15, 0.05, N.y);

    // Puddle coverage: procedural blobs (WHERE) on near-flat up-facing surfaces.
    // SSS placement is the default; the flow sim is the parked experimental source.
    ...
    // Sharp sky in puddles (LOD0); blurred off-puddle (but off-puddle reflK≈0 anyway).
    ...
    // Deep water = clear water BODY: cool dark tint.
    ...
    // SUN GLINT (SSFX specular_phong) — ripples shatter it into sparkles.
    ...
    // RING WAVE crests — bright leading edge of each expanding ripple (visible waves).
...
vec2 parallaxUV(vec2 uv, vec3 N, vec3 wp, out vec3 outN, out float outShadow, out float outAO)
{
    ...
    // More steps at grazing angle (where parallax is strongest); FEWER as POM
    // fades with distance. The step cut rides the SAME `fade` that shrinks amp:
    // as amp→0 the march has almost no depth to traverse, so fewer layers land
    // on the identical hit → invisible, but it halves the mid-distance march.
...
void main()
{
    ...
    if (sunMask > 0.005) {
        ...
        // VSM is now a screen-space, temporally-resolved mask → sample by screen UV
        // (ao_params.xy = 1/screenDims, same scale gtaoVis() uses).
