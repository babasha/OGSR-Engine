...
public:
    ...
    CRT rt_ReactiveMask;  // R8_UNORM            - DLSS reactive mask (1.0 = alpha-tested, 0.0 = normal)
    ...
    // DLSS Frame Generation render targets (display resolution, swapchain format)
    CRT rt_HudlessColor;  // Scene without UI (captured after tonemap, before HUD)
    CRT rt_InterpFrame;   // Interpolated frame output from DLSS-G
    CRT rt_RealFrame;     // Backup of final frame with UI (for double-present)
    ...
    // Shadow maps
    CRT rt_smap_depth;        // 3072x2048 - Sun cascade shadow atlas (D32_SFLOAT, 6x1024 in 3x2 grid)
    CRT rt_smap_depth_minmax; // 256x256   - MinMax optimization (R32G32_SFLOAT)
    ...
    u32 m_Width  = 0;
    u32 m_Height = 0;
    u32 m_ShadowMapSize = 2048;  // Legacy: used by spot light atlas (square assumption)
    u32 m_ShadowAtlasWidth  = 3072;  // Sun shadow atlas width
    u32 m_ShadowAtlasHeight = 2048;  // Sun shadow atlas height
...
public:
    ...
    // Render phases
    void phase_gbuffer_inner(VkCommandBuffer cmd);  // G-Buffer rendering code (no barriers)
    void RegisterGBufferPass(CFrameGraph& graph);   // Register G-Buffer as a graph GRAPHICS pass
    void phase_accumulator_inner(VkCommandBuffer cmd);  // setup + begin rendering (no entry barriers)
    void RegisterAccumulatorPass(CFrameGraph& graph);   // Register accumulator as a graph GRAPHICS pass
    void phase_combine();
    void phase_sky();                          // Standalone sky rendering (own begin/end)
    void phase_sky_draw(VkCommandBuffer cmd);  // Draw-only: no barriers/begin/end (E3 shared scope)
    void RegisterSkyPass(CFrameGraph& graph);  // EXTERNAL: standalone sky into rt_HDR
    void phase_clouds();                       // Standalone cloud rendering (own begin/end)
    void phase_clouds_draw(VkCommandBuffer cmd); // Draw-only: no barriers/begin/end (E3 shared scope)
    void RegisterCloudsPass(CFrameGraph& graph); // EXTERNAL: standalone clouds into rt_HDR
    // E3: sky+clouds share ONE vkCmdBeginRendering — no inter-draw barrier between them
    void BeginForwardSkyGroup(VkCommandBuffer cmd);  // entry barriers + begin rendering (depth READ_ONLY)
    void EndForwardSkyGroup(VkCommandBuffer cmd);    // end rendering + restore depth to ATTACH
    void RegisterSkyGroupPass(CFrameGraph& graph);   // EXTERNAL: sky+clouds as single pass
    void phase_forward();      // Phase 2.19: Forward pass (transparent objects)
    void phase_wallmarks_begin();  // Begin wallmarks render pass (rt_HDR + depth read-only)
    void phase_wallmarks_end();    // End wallmarks render pass
    void RegisterWallmarksPass(CFrameGraph& graph, std::function<void()> renderFn); // EXTERNAL: wallmarks into rt_HDR
    void phase_exposure();                              // Auto-exposure compute (rt_HDR → 1x1 R32F)
    void RegisterExposurePass(CFrameGraph& graph);      // Register exposure as a graph COMPUTE pass
    void RegisterTonemapPass(CFrameGraph& graph);       // Register tonemap + present as graph GRAPHICS passes
    void CaptureHudlessColorInner(VkCommandBuffer cmd); // barrier-free vkCmdCopyImage (barriers by graph)
    void RegisterE4Pass(CFrameGraph& graph, std::function<void()> hudFn); // E4: capture + HUD UI + present
    void RegisterCombinePass(CFrameGraph& graph);       // Register combine as a graph GRAPHICS pass
    void phase_dlss();                          // DLSS upscaling (rt_HDR → rt_DlssOutput)
    void RegisterDlssPass(CFrameGraph& graph); // Register DLSS as graph EXTERNAL pass
    ...
    void phase_smap_direct_inner(VkCommandBuffer cmd);  // all cascades, no barriers
    void RegisterSmapDirectPass(CFrameGraph& graph);    // Register sun shadow atlas as graph GRAPHICS pass
    ...
    void accum_point_shadowed(light* L);
    ...
    bool CreateSpotPipelineLazy();                  // Deferred spot light pipeline creation
    void DestroySpotPipeline();                     // Cleanup spot pipeline resources
    bool CreatePointPipelineLazy();                 // Deferred point light pipeline creation
    void DestroyPointPipeline();                    // Cleanup point pipeline resources
    bool CreatePointShadowPipelineLazy();           // Deferred shadowed point light pipeline creation
    void DestroyPointShadowPipeline();              // Cleanup shadowed point pipeline resources
...
private:
    ...
    // Spot light pipeline resources (dedicated layout - Phase 2.17.5)
    VkDescriptorSetLayout m_SpotShadowLayout = VK_NULL_HANDLE;   // Set 3: 1 sampler (shadow map)
    VkPipelineLayout m_SpotPipelineLayout = VK_NULL_HANDLE;
    VkPipeline m_SpotPipeline = VK_NULL_HANDLE;
    VkShaderModule m_SpotVS = VK_NULL_HANDLE;
    VkShaderModule m_SpotFS = VK_NULL_HANDLE;
    VkDescriptorSetLayout m_SpotEmptyLayout = VK_NULL_HANDLE;    // Placeholder for Set 2

    // Point light pipeline resources (dedicated layout, unshadowed)
    VkPipelineLayout m_PointPipelineLayout = VK_NULL_HANDLE;
    VkPipeline m_PointPipeline = VK_NULL_HANDLE;
    VkShaderModule m_PointVS = VK_NULL_HANDLE;
    VkShaderModule m_PointFS = VK_NULL_HANDLE;

    // Point light shadow pipeline resources (dedicated layout, with cubemap shadow)
    VkDescriptorSetLayout m_PointShadowLayout = VK_NULL_HANDLE;     // Set 3: 1 samplerCube
    VkDescriptorSetLayout m_PointEmptyLayout = VK_NULL_HANDLE;      // Set 2: empty placeholder
    VkPipelineLayout m_PointShadowPipelineLayout = VK_NULL_HANDLE;
    VkPipeline m_PointShadowPipeline = VK_NULL_HANDLE;
    VkShaderModule m_PointShadowFS = VK_NULL_HANDLE;
    VkSampler m_CubeShadowSampler = VK_NULL_HANDLE;                 // Non-comparison sampler for raw depth
...
public:
    ...
    void CreateFrameGenBuffers(u32 displayW, u32 displayH, VkFormat format);
    void DestroyFrameGenBuffers();
    void CaptureHudlessColor(VkCommandBuffer cmd);
    void BackupRealFrame(VkCommandBuffer cmd);
    bool m_bFrameGenBuffersReady = false;
