...
#include "vk_ring_allocator.h" // For VK::RingAlloc (bone region per-frame)
...
public:
    ...
    // === Shadow pass flag (skip pipeline/material/pushconstant rebind in Render()) ===
    bool                    m_bShadowPass = false;
    ...
    VK::CVulkanBuffer       m_BoneBuffer;                       // Fallback persistent SSBO (legacy)
    static const u32        MAX_BONES = 256;                    // Maximum bones per mesh
    static const u32        MAX_TOTAL_BONES = 16384;            // Total bone slots per frame (~1MB)
    Fmatrix*                m_BoneMapped = nullptr;             // CPU pointer to current frame's bone region
    u32                     m_BoneWriteOffset = 0;              // Per-skeleton bump cursor (in bone slots)
    VkBuffer                m_BoneBuffer_Current = VK_NULL_HANDLE;  // Ring buffer handle for this frame
    VkDeviceSize            m_BoneOffset_Current = 0;               // Base offset in ring for bone region
...
public:
    ...
    IC VkBuffer GetBoneBuffer() const { return m_BoneBuffer_Current != VK_NULL_HANDLE ? m_BoneBuffer_Current : m_BoneBuffer.m_Buffer; }
    IC bool IsBoneBufferValid() const { return m_BoneBuffer_Current != VK_NULL_HANDLE || m_BoneBuffer.IsValid(); }
    void SetBoneRegion(VK::RingAlloc alloc);
